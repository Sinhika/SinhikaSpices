...
import sinhika.bark.lib.ToolType;
...
public class ToolHelper implements IThingHelper
{
    ...
    /**
     * check for other tool materials added by other modules, and add tool info
    ...
    public void addSupplementalTools()
    {
        ...
        if (ModInfo.hasSimpleOres && ModInfo.so2_info.toolsExist)
        {
            for (EnumToolMaterial tm : ModInfo.so2_info.materialList)
            {
                if (materialsInUse.containsKey(tm)) continue;
                add(new ToolType(tm.toString().toLowerCase(), tm, no_string));
            } // end-for
            
            // simpleOresFusion depends on SimpleOres
            if (ModInfo.hasSimpleOresFusion && ModInfo.so2fusion_info.toolsExist)
            {
                for (EnumToolMaterial tm : ModInfo.so2fusion_info.materialList)
                {
                    if (materialsInUse.containsKey(tm)) continue;
                    add(new ToolType(tm.toString().toLowerCase(), tm, no_string));
                } // end-for
                
                // TODO akkamaddi's fusion stuff depends on SimpleOresFusion.
            } // end-if hasSimpleOresFusion
        } // end-if hasSimpleOres
