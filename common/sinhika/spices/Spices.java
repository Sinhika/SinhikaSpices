...
public class Spices
{
    ...
    public final static String[] barkTypes = { "oak", "spruce", "birch",
            "cinnamon" };
    private final static String[] capBarkTypes = { "Oak", "Spruce", "Birch",
            "Cinnamon" };
    ...
    private static final String[] toolTypes = { "wood", "stone", "iron",
            "gold", "diamond" };
    ...
    @SidedProxy(clientSide = "sinhika.spices.client.ClientProxy", serverSide = "sinhika.spices.CommonProxy")
    ...
    public void preInit(FMLPreInitializationEvent event)
    {
        ...
        Property logProp = config.get(Configuration.CATEGORY_GENERAL,
                "logLevel", "ALL");
        logProp.comment = "logging level ('ALL', 'OFF', 'infoST' to 'SEVERE'; per Java Logger API)";
        try
        {
        ...
        log.info("Config file " + event.getSuggestedConfigurationFile()
                + " read.");
        log.info("Logging level set to " + ModInfo.loggingLevel.toString());

        // get block IDs
        ModInfo.barkBlockID = config.getBlock("barkBlockID",
                ModInfo.DEFAULT_BLOCKID, "block ID for bark").getInt();
        ...
        for (int i = 0; i < barkTypes.length; i++)
        {
            ...
            ModInfo.barkItemID[i] = config.getItem(cfgKey,
                    ModInfo.DEFAULT_BASE_ITEMID + i).getInt();
            log.info("barkItemID[" + i + "]=" + cfgKey + "="
        ...
        for (int i = 0; i < toolTypes.length; i++)
        {
            ...
            ModInfo.toolItemID[i] = config.getItem("tools", cfgKey,
                    ModInfo.DEFAULT_BASE_ITEMID + barkTypes.length + i)
                    .getInt();
            log.info("toolItem[" + i + "]=" + cfgKey + "="
...
//            GameRegistry.registerCustomItemStack(barkBlockNames[i], barkBlockStack);
            ...
            log.info(oreDictBarkBlockNames[i]
        ...
        for (int i = 0; i < barkTypes.length; i++)
        {
            ...
            log.info("Actual barkItemID[" + i + "] =" + barkItems[i].itemID);
            barkItems[i].setUnlocalizedName(barkTypes[i] + "Bark");
            if (i != 3)
            {
                barkItems[i].func_111206_d(ModInfo.ID + ":" + barkTypes[i]
                        + "_bark");
            }
            else
            ...
            GameRegistry.registerCustomItemStack(barkTypes[i] + "Bark", 
        ...
        for (int i = 0; i < barkItems.length; i++)
        {
            for (int j = 0; j < toolItems.length; j++)
            {
                GameRegistry.addRecipe(new ShapelessOreRecipe(new ItemStack(
                        barkItems[i], harvestLevels[j] + 3), new ItemStack(
                        toolItems[j], 1, OreDictionary.WILDCARD_VALUE),
...
//            GameRegistry.addRecipe(new ShapelessOreRecipe(new ItemStack(
//                    barkItems[i], 9), oreDictBarkBlockNames[i]));

            // in-game item/block versions
            GameRegistry.addRecipe(new ShapedOreRecipe(barkBlockStack,
                    new Object[] { blockPattern, 'X', barkItems[i] }));
            GameRegistry.addRecipe(new ShapelessOreRecipe(new ItemStack(
                    barkItems[i], 9), barkBlockStack));
        } // end-for
    } // end addRecipes()
} // end class
